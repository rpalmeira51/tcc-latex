...
%In this work we approach the pentagon problem proving that every cubic graph with girth at least 17 has a homomorphism %to \(H_0^+ - 0\).
...
\begin{figure}
    \centering
    \input{Imagens/enhanced_clebsch_graph}
...
and hence, to solve \nesetril{} Pentagon Problem, it's enough to prove that there is a constant \(g_0\) such that every cubic graph with girth at least \(g_0\) has a homomorphism to \(H_0\),
i.e., a homomorphism to \(H\) that avoids the vertices in \(B_0\).


The motivation of this work is to show that every cubic graph of girth at least \(17\) has a homomorphism to the Enhanced Clebsch Graph that avoids a subset $\bads \subseteq B_0$, which improves the result of \cite{devos2011high} towards the Pentagon Problem. In the following, we proof the Theorem~\ref{thm:main} for $\bads=\{0\}$. 
