\chapter{The Algorithmic Argument}
...
We, also, show that positive results of the presented algorithms proves Conjecture~\refconj:main-conjecture} and present a simple complexity analysis. 
We use a python inspired pseudo-code to specify the algorithms for ease of reading (and also because we implemented a checker in Sage math see Chapter~\ref{chap:4}).
...
We say that that a configuration \emph{\(conf\) is bad} if equality holds, 
...
%Let $T$ and $T'$ be trees such that $T' \subseteq T$,  $c_1 \colon L(T) \to V(H)$ and $c_2 \colon L(T') \to V(H)$, we say that $c_1$ is \emph{descendent} of $c_2$ if there is a homomorphism $h \colon T \to H$ such that $h \in RH(c_1)$ and $h_{|T'} \in RH(c_2)$. 
...
TODO: Duvida para o fabio, provamos por contradicao aqui ? 

Naturally, it follows that, if all feasible configuration on a given tree $T$ of height $\ell$ are good, 
we proof Conjecture~\ref{conj:main-conjecture} for any $G$ with $g$ bigger than $\ell$.
As such 
...
and, finding a $\ell$ for which there are none, proof Conjecture~\ref{conj:main-conjecture}.
...
Let $conf$ be a configuration on $T$, 
...
We are now able introduce Algorithm~\ref{alg:naive-bottom-up:costs} (\textttt{CalculateCosts}) which is able calculate the costs of any configuration \(conf\) using a ``bottom-up'' recursive algorithm that 
...
Suppose we computed all bad configurations on a given tree $T$ of size $\ell$ as non empty set, 
computing the bad configurations for a size $\ell + 1 $ is prohibitively time-consuming and, for each bad configuration, there is at least 
one good configuration which colors just a few vertices differently.
Naturally, this motivates the search for 
a process that is able to, by local operations, 
modify, while avoiding increasing costs, configurations in order to change the coloring of certain vertices.
For this, we define \emph{replacements} as follows with deals with the simpler case of changing the color of a single vertex.

Let \(T\) be a complete binary tree rooted in \(r \in V(T)\), 
let \(c_0 \in V(\ch)\) a color and \(C\subseteq V(\chone) \setminus \{c_0\} \) a set of \emph{target} colors.
...
The following lemma comes naturally.
\begin{lemma}
    Let $T$ be a complete binary tree of size $\ell$ and $(c_0,~C)$ a replacement with respect to $T$. 
    Then~$(c_0,~C)$~is~a~replacement~for~any~complete~binary~tree~with~size~at~least~$\ell$.
\end{lemma}

As an example, in what follows, we present a simple family of replacements.
Let \(c_0\) be a color in $B_1$
and let \(C\) the set $V(\ch) \setminus N_{\ch}(c_0) \setminus \{c_0\}$, 
we call $(c_0, C)$ a \emph{trivial replacement} (See table \ref{table:replacements-height-1}).
...
We are able to determine if a pair $(c_0, C)$ is a replacement for a given complete binary tree $T$ computationally
...
Equivalently, we say that \(conf_2\) is \emph{similar to} \(conf_1\) and, additionally, \emph{similar in respect} to $x$.
A \emph{substitute} of \(conf\) is a configuration \(conf_*\) similar to \(conf\) such that $RC(conf_*) \leq RC(conf)$.
%
%Now, given a cubic graph $G$, a homomorphism $h \colon G \to CH$, a cubic tree $T \subseteq G$, let $conf = h|_{L(T)}$.
% Now, given a cubic tree $T \subseteq G$, and a homomorphism $h \colon T \to CH$, let $conf$ be a configuration completed by $h$.

Let be $(c_0, C)$ be a replacement, $conf$ a configuration of $T$, $x \in L(T)$ with $conf(x) = c_0$ and 
\(conf_{x\to c}\) the configuration obtained from \(conf\) by coloring \(x\) with \(c\).
If for each color $c \in C$, \(conf_{x\to c}\) is a substitute of $conf$, 
we define the set formed by these substitutes as a \emph{Substitution} of $conf$.
We call a configuration $conf$ solvable if we can construct a tree such that 
every node is a feasible configuration, 
the root is $conf$, 
the children of every internal node are a Substitution of the node's configuration 
and all node leafs are good configurations.
Additionally, we call this tree a \emph{Substitution tree}.

Note that, if for any given configuration $conf$, 
there is a Substitution such that all substitutes in the set are good or known solvable, 
we can form a Substitution tree rooted in $conf$.
Algorithm~\ref{alg:naive-replacements} (GetCurrentLevelSubstitutions) 
tries to find a \emph{Substitution tree} for each input bad configuration 
by determining, for each one, if there is a vertex $x$ for which there are substitutes in respect to $x$ 
such that there is a replacement $(c_0, C)$ where $x$ is colored with $c_0$ in the original configuration 
and every color in $C$ is the color of $x$ in at least one of the substitutes.
It does that, by first calculating the substitutes with respect to all vertices for each input bad configuration 
and then checking if there is replacement such that the set of these substitutes in respect to any vertex form a Substitution.
    ...
    for bad in bads: 
        ...
        for i in range(len(bad)):
            ...
            for newColorToTest in newColorsToTest:
                ...
                if(src > rc):
                    continue
                #not a root of a substitution tree
...
because no substitution tree was found in the previous steps and, therefore, can't be used as branch to construct new ones.
...
\section{Complexity and Conclusion}\label{complexity-analysys}
