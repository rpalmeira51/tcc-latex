\chapter{Disproving the conjecture}\label{chap6}
In this chapter,
we show that the algorithmic approach is unable to produce the desired results to prove Conjecture~\ref{conj:main-conjecture} 
...
We do this by introducing a process that generates a bad configuration for any complete cubic tree, regardless of its height.
...
%
\begin{lemma}
Let $T_b$ be a complete binary tree of height at least 2, 
...
If $IG$ is an induced copy of $P_4$, then there is only one possible color $c_u$ for $u$ and $c_v$ for $v$ such that $c_u$ and $c_v$ have common neighbors 
and, as consequence, there is no feasible configuration $conf^{(-1)}$ of $T_b^{(-1)}$ such that $conf^{(-1)}(u) \neq c_u$ or $conf^{(-1)}(v) \neq c_v$.
\end{lemma}
\begin{proof}
Suppose, for a contradiction, that $IG \simeq P_4$, but the statement does not hold.
...
Therefore all three edges of $IG$ are between $\{c_1,c_2\}$ and $\{c_3,c_4\}$, 
...
\begin{lemma}\label{lemma:pair-bad-coloring}
   ...
   In other words, the graph induced by $(N_{c_1}~\cap~N_{c_2})~\cup~(N_{c_3}~\cap~N_{c_4})$ is a $P_4$.
\end{lemma}
\begin{proof}
...
By Fact~\ref{fact:common_neighbors}, they have only two common neighbors and $d(c_u) = d(c_v) = 5$.
Without loss of generality, 
choose $c_w$ to be one of the neighbors of $c_v$ that is not a neighbor of $c_u$ and 
...
the subgraph induced by $\{c_u, c_v, c_w , c_1\}$ which is a $P_4$ with $\{c_u, c_v\}$, $\{c_u, c_w\}$, $\{c_v, c_1\}$ as non-edges, 
the first non-edge being the only only between the partitions.
\end{proof}

The following corollary follows naturally.
\begin{corollary}\label{coro:every-parents-configuration}
Let $conf_b$ be a feasible configuration of a binary tree $T_b^{(-1)}$ with height greater than 1. 
If $conf_b$ colors sibling vertices distinctly,
then
we can construct a configuration of $T_b$ for which $conf_b$ it's its only parents configuration.
\end{corollary}

\begin{lemma}\label{lemma:root-coloring}
   Let $T_b$ be a binary tree rooted in $r$ with height $l$ greater than 0 and $c_1$ and $c_2$ be two non adjacent vertices of $\ch$.
   There is a configuration of $T_b$ for which there are only two completions $h_1$ and $h_2$ 
   with $h_1(v) = h_2(v)$ for every $v$ in $V(T_b) \setminus \{r\}$, $h_1(r) = c_1$ and $h_2(r) = c_2$.
\end{lemma}
\begin{proof}
   The proof follows.
   If
   $T_b$ has height one ($l$ = 1) and, by consequence, we have vertices $v_1$ and $v_2$ children of $r$. 
...
\begin{lemma}\label{lemma:always-bad}
   Given any complete cubic tree $T$ of arbitrary large height $\ell > 1$ there is at least one bad configuration of $T$.
\end{lemma}
\begin{proof}
   ...
   Note that the intersection of the neighborhood of $c_2$, $c_v$ and $c_w$ is $b$ and the same holds for $c_3$, $c_v$ and $c_w$.
...
Lemma~\ref{lemma:always-bad} shows that for any arbitrary large complete cubic tree there is at least one bad configuration.
