...
Considerar que foi definido na introdução: 
1. Homomorfismo 
2. Árvore binaria/cubica completa ou não
3. C5
4. Grafo cúbico 
5. Cintura 
6. homomorfismos: se $G \to H$ e $H \to R$, então $G \to  R$
7. Vizinhanca com $N_{vertice}$
...
It is then natural to consider other computer search methods in order to improve Theorem~\ref{thm:DevosSamal11} and get a stronger result towards Conjecture~\ref{conj:pentagon-problem}.
We construct ours from the following observation:
...
To do this, we first establish a theoretical framework for locally recoloring the extended neighborhood of a vertex, that, due to the high girth condition, is isomorphic to a tree, 
in order to avoid undesired colors.
We, then, use this framework to prove the trivial result that we can always remove one of the undesired vertices from any homomorphism from a cubic graph of girth at least $17$ to a degenerate form of Clebsch graph that contains a few triangles.
Furthermore, we try unsuccessfully to use it to remove a color from any homomorphism from a cubic graph of girth at least $g$ to the Clebsch graph,
...
Recall that we are establishing a framework for exploring computer search methods that recolor homomorphisms to $\ch$ in order to avoid undesired colors.
For this, we need to be able to define a quantitative way to compare colorings on how they avoid these undesired colors and we use the simple way of counting undesired colors for each coloring.

Let \(G\) be a subcubic graph, let \(h\) be a homomorphism $G \to \ch$ and fix $B_1 \subseteq B_0$.
We define the \emph{cost} of $h$ as 
\[\cost(h)~=~|\{x\in V(G) : h(x)\in B_1\}|.\] 
...
and suppose there is a homomorphism \(h \colon G \to \ch\).
...
if \(d_T(u) = d_G(u)\) for every internal vertex of \(T\), 
...
\begin{lemma}\label{lemma:cost-improvement}
    Let \(G\) be a cubic graph for which there is a homomorphism \(h\) from \(G\) to \(CH\), let $T \subseteq G$ a cubic tree, $h'$ an $h$-compatible homomorphism from $T$ to $\ch$,
...
\begin{proof}
    ...
    \cost(h^*) 
        & = | \{x\in V(G) : h^*(x)\in B_1\} | \\
        & = | \{x\in V(G) \setminus V(T)) : h(x)\in B_1\} | + | \{x\in V(T) : h'(x)\in B_1\} | \\
        & = | \{x\in V(G) : h(x)\in B_1\} | -  |\{x\in V(T) : h(x)\in B_1\} | + cost(h') \\
        & = \cost(h) - | \{x\in V( T) : h(x)\in B_1\} | + cost(h') \\
...
Let \(T\subseteq G\) be a cubic tree,
and let \(h'\) be an \(h\)-compatible homomorphism of \(T\).
%
Consider the \((h,h')\)-modified homomorphism \(h^*\).
If \(\cost(h') < \cost(h|_T)\), then \(h^*\)
is a homomorphism \(G \to \ch\) for which, by Lemma~\ref{lemma:cost-improvement}, we have: 
\[\cost(h^*) = \cost(h) - \cost(h|_T) + \cost(h') < \cost(h).\]

Which is contradiction to the minimality of \(h\).
...
and \(h\) be a homomorphism from \(G\) to \(CH\) 
that minimizes \(\cost(h)\).
...
and an \(h\)-compatible homomorphism \(h'\) from \(T\) to \(CH\)
such that \(\cost(h') < \cost(h|_T)\).
\end{conjecture}

If Conjecture~\ref{conj:main-conjecture} holds,
then any homomorphism \(h \colon G \to CH\)
that minimizes \(\cost(h)\)
has \(\cost(h) = 0\).
